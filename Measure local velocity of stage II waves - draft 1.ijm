...
for(a=1; a<=nSlices; a++){
		...
		if(startFound && endFound){
			...
			waveFrontStack(waveStart, waveEnd, longestWave, grid, waveCount);
			waveCount += 1;
		}
...
function waveFrontStack(startSlice, endSlice, stackLength, grid, waveCount){
	...
	for(xGrid = 0; xGrid < grid; xGrid++){
		for(yGrid = 0; yGrid < grid; yGrid++){
			...
			for(a=1; a<=nSlices; a++){
				...
				else if((mean == waveMean && waveEnter <= nSlices) || (waveEnter <= nSlices && waveLeave == 0 && a == nSlices)) waveLeave = a;

				//If the box mean changes by more than 10% after a wave has left, count it as the new leave time (stalled wave)
				else if(mean > 1.1 * waveMean && waveLeave != 0) waveLeave = a;
