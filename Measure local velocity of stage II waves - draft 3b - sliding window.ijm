...
function waveFrontStack(startSlice, endSlice, stackLength, grid, waveCount, waveGap){
	...
	selectWindow("2-wave");
	getDimensions(width, height, channels, slices, frames);
	velocityArray = newArray(width*height);
	for(xGrid = 0; xGrid < width; xGrid++){
		for(yGrid = 0; yGrid < height; yGrid++){
			makeRectangle((xGrid-0.5)*(width/grid), (yGrid-0.5)*(height/grid), width/grid, height/grid);
			waveEnter = nSlices + 1;
			waveLeave = 0;
			waveMean = 0;
			for(a=1; a<=nSlices; a++){
			...
			velocityArray[xGrid*height + yGrid] = grid/(waveLeave - waveEnter);
		}
	}
	...
	for(a=0; a<width; a++){
		for(b=0; b<height; b++){
			setPixel(a, b, velocityArray[a*(width/grid) + b]);
		}
	}
