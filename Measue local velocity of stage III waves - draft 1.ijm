...
if(!matches(image, "Objects map of dF Stack.tif") && !matches(image, "Wave.tif")){
...
max = nResults;
getDimensions(width, height, channels, slices, frames);
newImage("Wave Velocity ("+ grid + "x" + grid +"array)", "8-bit black", step, step, max);
...
//			wait(100);
			...
			for(a=1; a<=nSlices; a++){
				...
				else if(mean < waveThreshold && waveEnter <= nSlices && waveLeave == 0) waveLeave = a;
	
				//If wave has not left the box at the end of the stack, it left the next frame
				else if(waveEnter <= nSlices && waveLeave == 0 && a == nSlices) waveLeave = a + 1;

				waveMean = mean;
				
			}
			//Store the wave velocity for that grid
			print(waveEnter + " " + waveLeave);
			velocityArray[xGrid*step + yGrid] = sqrt(area)/(waveLeave - waveEnter);
		}
	}
