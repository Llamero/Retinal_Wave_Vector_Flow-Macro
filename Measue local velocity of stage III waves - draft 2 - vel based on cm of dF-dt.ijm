...
max = nResults;
getDimensions(width, height, channels, slices, frames);
newImage("Mean Wave Velocity ("+ grid + "x" + grid +"array)", "32-bit black", step, step, max);
...
selectWindow("dF Stack");
getDimensions(width, height, channels, slices, frames);
selectWindow("Mean Wave Velocity ("+ grid + "x" + grid +"array)");
...
function waveFrontStack(startSlice, endSlice, grid, waveID, waveGap, width, height, step, waveThreshold){
	...
	meanVelocityArray = newArray(step*step);
...
//			wait(100);
			...
			for(a=0; a<waveXcm.length-1; a++){
				if(waveXcm[a] > -1){
					if(waveXcm[a+1] > -1){
						...
						waveVelSum += sqrt((xDist*xDist)+(yDist*yDist));
						xVecSum += xDist;
						yVecSum += yDist;
						velCount += 1;
					}
			...
			meanVelocity = waveVelSum / velCount;
			vecSumVel = sqrt((xVecSum*xVecSum)+(yVecSum*yVecSum))/velCount;
			vecSumAngle = atan2(yVecSum, xVecSum)*180/PI;

			
			//Store the wave velocity for that grid
			meanVelocityArray[xGrid*step + yGrid] = meanVelocity;
	...
	for(a=0; a<step; a++){
		for(b=0; b<step; b++){
			setPixel(a, b, meanVelocityArray[a*step + b]);
		}
	}
